...
class TestEditUser:
    ...
    def test_edit_user_data_success(self, edit_field, login_new_user):
        headers = {'Authorization': login_new_user[1]['accessToken']}
    ...
    def test_edit_user_password_success(self, login_new_user):
        headers = {'Authorization': login_new_user[1]['accessToken']}
        ...
        with allure.step('Отправка запроса на изменение пароля пользователя'):
            response = requests.patch(self.url, headers=headers, data=payload)
        payload_login = {'email': login_new_user[0]['email'], 'password': payload['password']}
        ...
        with allure.step('Отправка запроса на авторизацию пользователя c измененным паролем'):
            ...
            assert (response.status_code == 200 and response_login.status_code == 200)
    ...
    def test_edit_user_data_without_authorization_failure(self, edit_field, login_new_user):
